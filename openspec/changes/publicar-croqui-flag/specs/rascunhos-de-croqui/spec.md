...
### Requirement: Persistência de flag de publicação no Aresta Editor
O Aresta Editor SHALL manter a integridade local permitindo a pré-visualização de croquis não publicados.

#### Scenario: Aresta Editor roda deploy para preview local
- **WHEN** o Aresta Editor (no componente `croqui_experimental.py`) realiza uma build local
