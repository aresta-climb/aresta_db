...
- **Serviço Retransmissor em Nuvem (`previa.arestaclimb.com`)**: Implementação de retransmissor leve na Cloudflare que atua como intermediário de descoberta e proxy reverso de streaming HTTP-para-WebSocket, operando 100% em memória RAM sem persistência em banco de dados ou storage.
...
### New Capabilities
- `retransmissor-nuvem-previa`: Especificação do serviço de retransmissão na nuvem para gerenciamento de sessões efêmeras, anúncio de metadados de rede e streaming reverso em memória.
...
- **Infraestrutura em Nuvem (`aresta_backend` / Cloudflare)**: Novo pacote do Cloudflare Worker com testes automatizados TypeScript/Deno.
