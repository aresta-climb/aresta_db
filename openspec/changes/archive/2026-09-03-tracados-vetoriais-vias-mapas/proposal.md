...
- **Editor de Mapas Desktop (Integrado à QUndoStack):**
  ...
  - Seletor de cores com paleta de alto contraste para rocha (Vermelho, Laranja, Amarelo, Verde Lima, Ciano, Roxo, Branco) e seletor customizado.
  ...
  - Todas as mutações de nós, linhas e cores mediadas obrigatoriamente por comandos `QUndoCommand` na pilha global de histórico.
...
### New Capabilities
- `tracados-vetoriais-mapas`: Modelo de dados, semântica de nós de escalada, cores de elementos visuais, biblioteca matemática de interpolação Spline Catmull-Rom para Bézier e pipeline de compilação em SVG Path GPU-ready.

### Modified Capabilities
- `editor-mapas`: Adição da ferramenta visual de caneta/traçado de vias, seletor de cores, manipulação e movimentação interativa de nós com spline em tempo real, alternância de tipos de nó e suporte a histórico (undo/redo).

## Impact

- **Protobuf (`aresta_api/proto/croqui.proto`):** Adição de mensagens `LinhaTrajeto`, `DadosConteudoLinha`, `DadosCompiladosLinha`, `NoTrajeto`, `MarcadorCompilado` e campo `cor` em `PontoDeInteresse`. Nomenclatura 100% em português brasileiro.
- **Core da Aplicação (`editor/core/` / `aresta_api/core/`):** Biblioteca pura de interpolação Spline e expansão de `GeometriaPOI` para suportar `cor` e `LinhaTrajeto`.
- **Views do Editor (`editor/views/widget_editor_mapas.py`):** Novo item gráfico `ItemTrajetoLinha`, seletor de cores e ferramenta de caneta com feedback em tempo real.
