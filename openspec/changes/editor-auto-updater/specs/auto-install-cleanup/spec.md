...
- **THEN** o sistema copia a si mesmo para `%LOCALAPPDATA%\ArestaEditor\`, substitui o arquivo original por um atalho `.lnk`, cria o recibo `cleanup_folder.txt` e reinicia do local oficial.

### Requirement: Restrictive Garbage Collection
The system SHALL limpar instâncias obsoletas (`ArestaEditor.old.exe`) sob restrições rígidas de nome de arquivo para prevenir exclusões arbitrárias.
