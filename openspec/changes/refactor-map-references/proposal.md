...
Adicionalmente, os sub-agentes de IA (workflow `processar_croqui_completo`) que preenchem os croquis na Etapa 2 de forma automática precisam saber qual formato estruturado gerar.
...
- Atualização das *Skills* dos agentes de processamento (`@converter_parte_croqui_para_markdown` e `@preencher_croqui_yaml`) para que, durante o parsing de PDFs, injetem as referências dentro dos objetos de mapa no YAML em vez das escaladas.
...
- `orquestrador-extracao`: As skills de extração paralela serão ajustadas para abraçar a nova estrutura de `referencias` na Fase 2. (Nota: não criei um spec file separado para isso, pois é um ajuste nas instruções e templates das skills atuais).
...
- **Agentes de Automação**: As descrições em `SKILL.md` de conversão e compilação de croquis precisarão ser reescritas refletindo as exclusões dos campos antigos.
