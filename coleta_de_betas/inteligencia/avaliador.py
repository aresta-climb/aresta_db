...
def parsear_resposta_llm(conteudo_resposta: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if match_arr:
            try:
                dados_fallback: Any = json.loads(match_arr.group(0))
                if isinstance(dados_fallback, list):
                    return [item for item in dados_fallback if isinstance(item, dict)]
            except Exception:
                pass
    return []
...
def avaliar_candidatos(
    ...
    if client_llm and hasattr(client_llm, "gerar_texto"):
        resposta_texto: str = str(client_llm.gerar_texto(prompt))
        ...
        for m in midias:
            if m.url in mapa_avaliacoes:
                ...
                m.resultado_llm.llm_reasoning = str(dados_av.get("justificativa", ""))
